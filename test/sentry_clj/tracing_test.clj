(ns sentry-clj.tracing-test
  ...
  (:import
   [io.sentry CustomSamplingContext Scope Sentry SentryOptions SentryTracer Span SpanStatus TracesSamplingDecision TransactionContext]
   [io.sentry.protocol Request SentryId]))

(defn ^:private get-test-options ^SentryOptions
  ([] (get-test-options {}))
  ([{:keys [traces-sample-rate] :as _opts}]
...
(defn ^:private get-test-sentry-tracer ^SentryTracer
  []
  (let [sentry-options (get-test-options)
        _sentry (Sentry/init ^SentryOptions sentry-options)
        traces-sampling-decision (TracesSamplingDecision. true 1.0)
        tr (SentryTracer. (TransactionContext. "name" "op" traces-sampling-decision) (Sentry/getCurrentScopes))]
...
(defexpect span-status-test
  (expecting "extract span status by keyword"
...
(defexpect compute-custom-sampling-context-test
  (expecting "compute custom-sampling-context"
...
(defexpect swap-transaction-status-test
  (expecting "change transaction status"
...
(defexpect with-start-child-span-test
  (expecting "when a child span is started and works correctly, the evaluation result is given and span status is OK"
    (let [tr (get-test-sentry-tracer)]
      (expect 1 (sut/with-start-child-span "op" "desc" (+ 1 1) (* 1 1)))
      (expect (.getStatus ^Span (nth (.getChildren tr) 0)) (:ok sut/span-status))
      (sut/finish-transaction! tr)))
  (expecting "when a child span is started with no tracing config, the evaluation result is only given."
    (let [sentry-options (get-test-options)]
      (Sentry/init ^SentryOptions sentry-options)
      (expect 1 (sut/with-start-child-span "op" "desc" (+ 1 1) (* 1 1)))))
  (expecting "when a child span is started and throw exceptions, span status is INTERNAL_ERROR"
    (let [tr (get-test-sentry-tracer)]
      (try
...
(defexpect start-transaction-test
  (expecting "when trace option isn't set, trace transaction isn't created"
    (let [sentry-options (get-test-options)]
      (Sentry/init ^SentryOptions sentry-options)
      (let [tr (sut/start-transaction "op" (CustomSamplingContext.) nil)]
        (expect io.sentry.NoOpTransaction tr)
        (expect nil (sut/finish-transaction! tr)))))
  (expecting "when there is not a sentry-trace-header, new trace transaction is created"
    (let [sentry-options (get-test-options {:traces-sample-rate 1.0})
          description "http.server"]
      (Sentry/init ^SentryOptions sentry-options)
      (let [^SentryTracer tr (sut/start-transaction description (CustomSamplingContext.) nil)]
        (expect (.getOperation tr) description)
        (expect (complement nil?) (.getTraceId (.toSentryTrace tr)))
        (expect nil (sut/finish-transaction! tr)))))
  (expecting "when there is a sentry-trace-header, exist trace transaction is gotten"
    (let [sentry-options (get-test-options {:traces-sample-rate 1.0})
          sentry-trace-header "f084f508efca47e9a3313851d4d8b7a2"
          sentry-span-id "00f067aa0ba902b7"]
      (Sentry/init ^SentryOptions sentry-options)
      (let [^SentryTracer tr (sut/start-transaction "foo" (CustomSamplingContext.) (str sentry-trace-header "-" sentry-span-id "-1"))]
